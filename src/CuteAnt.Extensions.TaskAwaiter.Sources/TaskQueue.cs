...
//#if CLIENT
//using CuteAnt.Osin.SignalR.Client.Infrastructure;
//#endif
...
namespace CuteAnt.AsyncEx
{
  ...
  internal sealed class TaskQueue
  {
    ...
    public Task Enqueue(Func<object, Task> taskFunc, object state)
    {
      ...
      lock (_lockObj)
      {
        if (_drained) { return _lastQueuedTask; }
    ...
    [SuppressMessage("Microsoft.Performance", "CA1811:AvoidUncalledPrivateCode", Justification = "This is shared code")]
    public Task Enqueue(Func<Task> taskFunc) => Enqueue(state => ((Func<Task>)state).Invoke(), taskFunc);
