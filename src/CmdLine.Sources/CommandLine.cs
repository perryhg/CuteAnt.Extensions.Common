...
(?<Value>.*)\Z";
    ...
    private static extern FileType GetFileType(SafeFileHandle handle);
    [DllImport("kernel32.dll")]
    private static extern SafeFileHandle GetStdHandle(StdHandle std);
...
#else
               string.Format(TokenizeExpressionFormat, GetCaseSensitiveOption(), string.Join(string.Empty, CommandSeparators.ToArray()), string.Join(string.Empty, ValueSeparators.ToArray()));
#endif
      }
    }
    ...
    public static char PromptKey(string prompt, params char[] allowedKeys)
    {
      ...
      do
      {
        try
        {
          ...
          keyChar = ToLower(Console.ReadKey(true).KeyChar);
